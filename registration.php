...
$_POST['first_name'] = trim($_POST['first_name']);
	try {
		...
		if((is_numeric($_POST['mobile']))==FALSE) {
			throw new Exception('Mobile Number cannot contain alphabets.');
			
		}
		if(((int) (log($_POST['mobile'], 10) + 1))!=10) {
			throw new Exception('Mobile Number should have 10 digits.');
	...
	if(strcmp((string)$_POST['password'],(string)$_POST['cpassword'])!==0) {
		...
		<div class="testbox">
			...
			<?php
				...
				if(isset($success_message)) {echo $success_message;}
				  ...
				  <input type="password" name="password" id="name" placeholder="Password" value="'.$u_pass.'" required/>
				  <label id="icon" for="name"><i class="icon-shield"></i></label>
				  <input type="password" name="cpassword" id="name" placeholder="Confirm Password" value="'.$u_cp.'" required/>
				  <div class="gender">
